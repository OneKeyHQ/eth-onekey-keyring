...
const HDCore = require('@onekeyfe/hd-core');

const { getDeviceType } = HDCore;
...
const IFRAME_SRC = 'https://jssdk.onekey.so/0.1.30/';
 ...
 *
 * The OneKey Model 1 does not currently support constructing the hash on the device,
 ...
 */
const transformTypedData = (data, metamaskV4Compat) => {
  if (!metamaskV4Compat) {
    throw new Error(
      'OneKey: Only version 4 of typed data signing is supported',
    );
  }
...
class OneKeyKeyring extends EventEmitter {
  constructor(opts = {}) {
    ...
    HardwareSDK.init({
      ...
      HardwareSDK.on(HDCore.DEVICE.FEATURES, (event) => {
        if (event && event.payload && event.payload.features) {
          this.model = event.payload.features.model;
          this.deviceType = getDeviceType(event.payload.features);
        }
      });
    });
  }
  ...
  async getDevice() {
    return new Promise((resolve, reject) => {
      HardwareSDK.requestWebUsbDevice()
        .then((response) => {
          if (!response.success) {
            reject(
              new Error(
        ...
        .then((response) => {
          if (response.success) {
            this.hdk.publicKey = Buffer.from(
              response.payload.node.public_key,
              'hex',
            );

            this.hdk.chainCode = Buffer.from(
              response.payload.node.chain_code,
              'hex',
            );
   ...
   * @param {Transaction} tx - Instance of either new-style or old-style ethereumjs transaction.
   * @param {(EthereumSignedTx) => Transaction} handleSigning - Converts signed transaction
  ...
  async signTypedData(address, data, { version }) {
    ...
    // OneKey is stricter than @metamask/eth-sig-util in what it accepts
    const {
    ...
    const response = await HardwareSDK.evmSignTypedData(
      this.device.connectId,
      this.device.deviceId,
      {
        path: this._pathFromAddress(address),
        ...
        domainHash: domain_separator_hash,
