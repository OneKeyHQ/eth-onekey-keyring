...
const HardwareSDK = require('@onekeyfe/hd-web-sdk');
const {
  TransactionFactory,
...
describe('OneKeyKeyring', function () {
  ...
  describe('constructor', function () {
    it('constructs', function (done) {
      const t = new OneKeyKeyring({ hdPath: `m/44'/60'/0'/0` });
  ...
  describe('unlock', function () {
    ...
    it('should call HardwareSDK.getPublicKey if we dont have a public key', async function () {
      sinon
        .stub(HardwareSDK, 'getPublicKey')
      ...
      assert(HardwareSDK.getPublicKey.calledOnce);
    });
  });
  ...
  describe('signTransaction', function () {
    it('should pass serialized transaction to onekey and return signed tx', async function () {
      sinon.stub(HardwareSDK, 'evmSignTransaction').callsFake(() => {
      ...
      assert(HardwareSDK.evmSignTransaction.calledOnce);
    });
    ...
    it('should pass serialized contract deployment transaction to onekey and return signed tx', async function () {
      ...
      assert.equal(returnedTx.common.chainIdBN().toString('hex'), '1');
      assert(HardwareSDK.evmSignTransaction.calledOnce);
      assert.deepEqual(HardwareSDK.evmSignTransaction.getCall(0).args[0], {
        path: `m/44'/60'/0'/0/0`,
        transaction: {
    ...
    it('should pass correctly encoded EIP1559 transaction to onekey and return signed tx', async function () {
      ...
      sinon.stub(HardwareSDK, 'evmSignTransaction').callsFake((params) => {
  ...
  describe('signMessage', function () {
    it('should call HardwareSDK.ethereumSignMessage', function (done) {
      sinon
        .stub(HardwareSDK, 'ethereumSignMessage')
        .callsFake(() => Promise.resolve({}));
      ...
      setTimeout(() => {
        assert(HardwareSDK.ethereumSignMessage.calledOnce);
        done();
      }, SIGNING_DELAY);
    });
  });
  ...
  describe('signTypedData', function () {
    ...
    it('should call HardwareSDK.evmSignTypedData', async function () {
      sinon.stub(HardwareSDK, 'evmSignTypedData').callsFake(async () => ({
        success: true,
      ...
      assert(HardwareSDK.evmSignTypedData.calledOnce);
      sinon.assert.calledWithExactly(HardwareSDK.evmSignTypedData, {
        path: "m/44'/60'/0'/0/0",
        data: {
          // Empty message that hardware-js-sdk/EIP-712 spec accepts
